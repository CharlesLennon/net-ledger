  ...
  function processLocation(
    ...
    location.children.forEach(childLoc => {
      ...
      if (location.layout_direction === 'horizontal') {
        currentX +=
          (childGroup.size ? childGroup.size[0] : 400) + SPACING_CONFIG.GROUP_VERTICAL_SPACING;
      } else {
        currentY +=
    ...
    if (Math.abs(deltaX) > WIRE_ROUTING_CONFIG.MIN_DISTRIBUTED_DISTANCE) {
      if (WIRE_ROUTING_CONFIG.ENABLE_CHANNEL_ROUTING) {
        if (isPowerConnection) {
          ...
          const channelOffset = channelIndex * WIRE_ROUTING_CONFIG.CHANNEL_SPACING;
          const shouldRouteLeft =
            (startDir === (LiteGraph?.LEFT || 3) && endDir === (LiteGraph?.LEFT || 3)) ||
            (startDir === (LiteGraph?.RIGHT || 4) &&
              endDir === (LiteGraph?.LEFT || 3) &&
              Math.abs(deltaX) < 200);
          if (shouldRouteLeft) {
            const routingX = WIRE_ROUTING_CONFIG.LEFT_CHANNEL_X + channelOffset;
            points.push({ x: routingX, y: startY });
        ...
        if (isPowerConnection) {
          ...
          const routingX =
            startX -
            WIRE_ROUTING_CONFIG.NON_POWER_OFFSET_LEFT -
            WIRE_ROUTING_CONFIG.POWER_OFFSET_LEFT -
            powerChannelIndex * WIRE_ROUTING_CONFIG.POWER_CHANNEL_OFFSET;
          points.push({ x: routingX, y: startY });
          points.push({ x: routingX, y: endY });
          points.push({ x: endX, y: endY });
        } else {
          ...
          if (shouldRouteLeft) {
            const routingX =
              startX -
              WIRE_ROUTING_CONFIG.NON_POWER_OFFSET_LEFT -
              networkChannelIndex * WIRE_ROUTING_CONFIG.NETWORK_CHANNEL_OFFSET;
            points.push({ x: routingX, y: startY });
            points.push({ x: routingX, y: endY });
            points.push({ x: endX, y: endY });
          } else {
            const routingX =
              startX +
              WIRE_ROUTING_CONFIG.NON_POWER_OFFSET_RIGHT +
              networkChannelIndex * WIRE_ROUTING_CONFIG.NETWORK_CHANNEL_OFFSET +
              wireOffset;
            points.push({ x: routingX, y: startY });
            points.push({ x: routingX, y: endY });
